...
public class NewestListController extends BaseController implements SwipeRecyclerView.OnRefreshAndLoadListener {
	...
	protected View initView(Context context) {
		...
		// 注入
		x.view().inject(view);
		View mPopupView = LayoutInflater.from(context).inflate(R.layout.popup_window_more, null);
		x.view().inject(mPopupView);
		...
		//设置true为点击返回或者空白处 popupWindow消失
		mPopupWindow = new PopupWindow(mPopupView, ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, true);
		ll_report = (LinearLayout) mPopupView.findViewById(R.id.ll_pop_more_report);
		ll_save = (LinearLayout) mPopupView.findViewById(R.id.ll_pop_clue);
		//设置下拉进度条颜色
...
//		mSwipeRefreshWidget.setOnRefreshListener(this);//不需要再设置刷新功能
		...
		mSwipeRefreshWidget.setProgressViewOffset(true, -500, (int) TypedValue
				.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, mContext.getResources()
						.getDisplayMetrics()));
		mRecyclerView.setOnRefreshAndLoadListener(mSwipeRefreshWidget, this);
		mRecyclerView.setHasFixedSize(true);//如果item大小一样,可以提高效率
...
//		模拟数据显示 TODO:
	...
	private class NewsListAdapter extends SwipeRecyclerViewAdapter implements View.OnClickListener {
		...
		public int GetItemCount() {
			return mNewsDatas != null ? mNewsDatas.size() : 0;
		}
